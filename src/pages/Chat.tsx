...
const Chat = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    let promptParaBackend = inputValue;
    const displayMessage = inputValue || `Analisar o arquivo: ${fileName}`;

    if (pdfContent) {
      if (inputValue.toLowerCase().includes('resumo') || !inputValue.trim()) {
        promptParaBackend = PdfProcessor.createSummaryPrompt(pdfContent, pdfInfo?.pages || 0);
      } else {
        promptParaBackend = PdfProcessor.createAnalysisPrompt(pdfContent, pdfInfo?.pages || 0, inputValue);
      }
    }
    ...
    <div className="h-screen max-h-screen bg-background flex flex-col">
      <header className="border-b border-border sticky top-0 bg-background/95 backdrop-blur z-10">
         <div className="container mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <div className="md:hidden flex items-center gap-1"><ThemeToggle /><Sheet><SheetTrigger asChild><Button variant="ghost" size="icon"><Menu className="h-5 w-5" /></Button></SheetTrigger><SheetContent side="right" className="w-[320px] p-0 flex flex-col"><SheetHeader className="p-4 border-b"><SheetTitle>Menu</SheetTitle></SheetHeader><div className="p-4 space-y-4 border-b"><UserProfile /><ModelSelector onModelSelect={setSelectedModel} selectedModel={selectedModel} /></div><div className="flex-1 flex flex-col overflow-hidden"><SheetClose asChild><div><ConversationSidebar conversations={conversations} currentConversationId={currentConversationId} onSelectConversation={openConversation} onNewConversation={createNewConversation} onDeleteConversation={deleteConversation} onToggleFavorite={toggleFavoriteConversation} onRenameConversation={renameConversation} isMobile={true} /></div></SheetClose></div></SheetContent></Sheet></div>
        </div>
      </header>
